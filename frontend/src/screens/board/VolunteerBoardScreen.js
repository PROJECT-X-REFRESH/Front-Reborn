...
const VolunteerBoardScreen = ({ navigation }) => {
  ...
  const loadBoardData = useCallback(async () => {
    ...
    try {
      if (selectedScreen === 'popular') {
        boardData = await fetchPopularBoards('VOLUNTEER');
      } else if (selectedScreen === 'like') {
        boardData = await fetchLikedBoards('VOLUNTEER');
      } else {
        boardData = await fetchBoardList({ type: BOARD_TYPE.VOLUNTEER });
      }
  ...
  return (
    <Container>
      ...
      <WriteButton
        onPress={() =>
          navigation.navigate('BoardWrite', { category: 'VOLUNTEER' })
        }>
...
export default VolunteerBoardScreen;
