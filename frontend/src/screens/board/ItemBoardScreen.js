...
const ItemBoardScreen = ({ navigation }) => {
  ...
  const loadBoardData = useCallback(async () => {
    ...
    try {
      if (selectedScreen === 'popular') {
        boardData = await fetchPopularBoards('SHARE');
      } else if (selectedScreen === 'like') {
        boardData = await fetchLikedBoards('SHARE');
      } else {
        boardData = await fetchBoardList({ type: BOARD_TYPE.SHARE });
      }
  ...
  return (
    <Container>
      ...
      <WriteButton
        onPress={() =>
          navigation.navigate('BoardWrite', { category: 'SHARE' })
        }>
